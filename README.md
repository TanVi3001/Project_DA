...
&#x20; \* `RAL\_full`: Tên phân loại dòng gen (105 loại).

&#x20; \* `datetime`: Thời gian ghi nhận dữ liệu cảm biến.

&#x20; \* `batch`: Chu kỳ ngày đo lường (day1, day2...).

&#x20; \* `act\_sum`: Tổng mức độ hoạt động ghi nhận được.
...
1\. Dòng gen (`RAL\_full`) nào có mức độ vận động trung bình cao nhất và thấp nhất?
...
\* \*\*Kiểm tra tính toàn vẹn:\*\* Xác nhận cột `act\_sum` không chứa giá trị âm vô lý (Min = 0).



\### 4. ANALYZE (Phân tích dữ liệu)

\* Sử dụng phép tính gộp nhóm (`GROUP BY`) để tính trung bình mức độ hoạt động (`act\_sum` mean) theo từng dòng gen (`RAL\_full`).
